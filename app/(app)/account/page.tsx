...
export default async function AccountPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-8">
        ...
        {entitlements.isPro ? (
          ...
          <div className="mt-4">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              ...
              <Link
                href="/pricing"
                className="rounded-lg border border-surface-border px-5 py-2 text-sm text-ink-soft transition hover:border-brand-600"
              >
                Compare plans
              </Link>
            </div>
      ...
      {user.role === "admin" && (
        <section className="mt-6 rounded-xl border border-surface-border bg-white p-6">
          <h2 className="font-semibold text-ink">Admin tools</h2>
          <p className="mt-1 text-sm text-ink-soft">
            Grant Pro access directly, without a payment, for comped or partner
            accounts.
          </p>
          <AdminGrant />
        </section>
